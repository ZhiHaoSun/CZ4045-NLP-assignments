...
def generate_examples(windows, nb_append=None, nb_skip=0, verbose=True):
    ...
    for window in windows:
        ...
        if skipped < nb_skip:
            ...
            yield (feature_values_lists, labels, [token.word for token in window.tokens])
...
class Article(object):
    ...
    def __init__(self, text):
        ...
        tokens_str = [token_str.strip() for token_str in word_tokenize(text) if len(token_str.strip()) > 0]
