...
def test_on_articles(identifier, articles, nb_append=None):
    ...
    all_tokens = []
    model_labels = []
    for fvlist, labels, tokens in generate_examples(windows, nb_append=nb_append):
    ...
    predicted_label_chains = []
    for fv in all_feature_values_lists:
        tags = tagger.tag(fv)
        predicted_label_chains.append(tags)
        model_labels.extend(tags)
    result = zip(all_tokens, model_labels)
    ...
    while i < len(result) - 1:
        if result[i][1] != 'O':
            j = i+1
            while j < len(result) and result[j][1] != 'O':
                j = j + 1
            entities = result[i][0]
            for k in range(i+1, j):
                entities += ' ' + result[k][0]
            i = j
            result_tokens.append(entities)
        else:
